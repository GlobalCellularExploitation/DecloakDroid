...
import com.decloakdroid.hooks.HookList;
import com.decloakdroid.custom_hooks.CustomHookList;
	...
	protected static void _hookMethodImpl(final MS.MethodPointer old,
		...
		if (packageName != null && dataDir != null && 
			...
			try {
				...
				if (LoadConfig.getInstance().getHookTypes().contains("SAVE DB") &&
						 !LoadConfig.getInstance().getHookTypes().contains("SQLite"))
					elemConfig.getFunc().enableDBlogger();

				if (LoadConfig.getInstance().getHookTypes().contains("SAVE LOGS"))
					elemConfig.getFunc().enableFilelogger();
				else
					elemConfig.getFunc().disableFilelogger();
